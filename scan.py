...
# Max meat scale limit is 1500 g only.
LOAD_CELL_MAX_KG = 1.5
LOAD_CELL_MAX_G = int(LOAD_CELL_MAX_KG * 1000)
        ...
        if text.startswith("W,"):
            ...
            if len(parts) >= 3:
                try:
                    ...
                    # Safety cap in Python also, even if Arduino sends above the limit.
                    if weight_kg > LOAD_CELL_MAX_KG:
                        weight_kg = LOAD_CELL_MAX_KG
                        weight_g = float(LOAD_CELL_MAX_G)
...
class ScanScreen(tk.Frame):
    ...
    def get_weight_module_status_text(self, weight_kg):
        ...
        if weight_kg >= LOAD_CELL_MAX_KG:
            return "MAX WEIGHT REACHED", ERROR_COLOR
    ...
    def update_weight_module(self):
        ...
        if weight_kg >= LOAD_CELL_MAX_KG:
            self.weight_value_label.config(
                text=f"{LOAD_CELL_MAX_G} g",
                fg=ERROR_COLOR,
            )
        else:
            self.weight_value_label.config(
                text=f"{weight_grams} g",
                fg=TEXT_COLOR,
            )
    ...
    def perform_capture_and_check(self, manual=False, is_still=False):
        ...
        if mode == "meat":
            ...
            # 3) The meat weight must not reach/exceed the 1500g max limit.
            ...
            if current_weight_kg >= LOAD_CELL_MAX_KG:
                self.show_temporary_status("MAX WEIGHT REACHED", ERROR_COLOR, 1500)
                return
