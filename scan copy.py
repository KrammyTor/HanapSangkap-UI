...
class ScanScreen(tk.Frame):
    ...
    def get_current_meat_weight_kg(self):
        """
        Returns the current meat weight.

        Important fix:
        If the Arduino/serial scale is disconnected or still not sending valid W lines,
        do not permanently block meat scanning with 0 kg. Use the controller temporary
        value only as a fallback so the demo still works while the scale is being fixed.
        """
        ...
        if weight is None:
            weight = getattr(self.controller, "temporary_meat_weight_kg", TEMP_MEAT_WEIGHT_KG)
            try:
                weight = float(weight)
            except Exception:
                weight = TEMP_MEAT_WEIGHT_KG

        if weight < 0:
            weight = 0.0

        if weight > LOAD_CELL_MAX_KG:
            weight = LOAD_CELL_MAX_KG
    ...
    def update_weight_module(self):
        ...
        self.weight_after_id = self.after(700, self.update_weight_module)
    ...
    def perform_capture_and_check(self, manual=False, is_still=False):
        ...
        if mode == "meat":
            ...
            if self.get_current_meat_weight_kg() < MIN_MEAT_WEIGHT_KG_FOR_SCAN:
