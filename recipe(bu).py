...
DISH_CARD_WIDTH = 350
DISH_CARD_HEIGHT = 295

DISH_TITLE_AREA_HEIGHT = 27
...
# Boneless meat = 250g per person
# Bone-in meat = 350g per person
BONELESS_GRAMS_PER_SERVING = 250
BONE_IN_GRAMS_PER_SERVING = 350
...
class RecipeScreen(tk.Frame):
    ...
    def _round_weight_to_servings(self, weight_g, grams_per_serving):
        ...
        # Example: 300g // 250g = 1 serving, 1000g // 250g = 4 servings.
    ...
    def get_filtered_dishes(self):
        ...
        def ingredient_matches(recipe_ingredients, detected_name):
            detected_name = (detected_name or "").strip().upper()

            if not detected_name:
                return False

            detected_meat_type = meat_type_from_text(detected_name)

            for recipe_ingredient in recipe_ingredients:
                recipe_ingredient = (recipe_ingredient or "").strip().upper()

                if not recipe_ingredient:
                    continue

                if detected_name == recipe_ingredient:
                    return True

                if detected_name in recipe_ingredient or recipe_ingredient in detected_name:
                    return True

                if detected_meat_type and meat_type_from_text(recipe_ingredient) == detected_meat_type:
                    return True
        ...
        if detected_items:
            ...
            for dish_name in region_matches:
                ...
                if all(ingredient_matches(recipe_ingredients, item) for item in detected_items):
            ...
            for dish_name in region_matches:
                ...
                if ingredient_matches(recipe_ingredients, ingredient):
                    dishes.append(dish_name)
    ...
    def _create_dish_card(self, parent, dish_name):
        ...
        image_holder.pack(anchor="center", padx=8, pady=(5, 1))
        ...
        info.pack(fill="both", expand=True, padx=8, pady=(0, 3))
    ...
    def _create_favorite_row(self, parent, dish_name):
        ...
        # ------------------------------------------------------------
        # UNFAVORITE BUTTON ON THE LEFT SIDE - CENTERED
        # ------------------------------------------------------------
        ...
        # ------------------------------------------------------------
        # IMAGE AFTER THE BUTTON - WITH CLEAN BORDER
        # ------------------------------------------------------------
